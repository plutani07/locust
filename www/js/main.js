...
/* The splash covers the boot read. It comes away once the library is on
   screen, but never so fast that it reads as a flicker — and a failsafe
   takes it away even if boot throws, so a bad read can't leave you
   staring at a logo. */
const bootAt = Date.now();
let splashGone = false;

/* Android shows a native splash (a still of the mark on the same black)
   until we say otherwise. Hand over on the first frame the web splash is
   actually painted — earlier leaves a black gap, later and the animation
   plays behind a static image nobody sees. */
function handoff(){
  const sp = window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.SplashScreen;
  if (!sp) return;
  requestAnimationFrame(() => requestAnimationFrame(() => {
    try { sp.hide({ fadeOutDuration: 220 }); } catch {}
  }));
}
handoff();
...
function hideSplash(){
  ...
  const el = $('#splash');
  if (!el) return;
  const hold = parseInt(el.dataset.hold, 10) || 1250;
  const wait = Math.max(0, hold - (Date.now() - bootAt));
  setTimeout(() => {
